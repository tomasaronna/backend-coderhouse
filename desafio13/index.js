...
passport.use(
  ...
  new LocalStrategy(
    { passReqToCallback: true },
    (req, username, password, done) => {
      const usuarioExistente = modelUsuarios.find({
        $and: [{ user: username }, { password: password }],
      });
      loggerConsola.info(usuarioExistente);
      if (usuarioExistente) {
        loggerWarn.warn("Usuario existente");
        return done(null, false);
      } else {
        modelUsuarios.insertOne({ user: username }, { password: password });
        done(null, { user: username });
      }
    }
  )
...
// inicio del servidor
app.listen(PUERTO, () => {
  loggerConsola.info(`Servidor levantado en el puerto: ${PUERTO}`);
});
